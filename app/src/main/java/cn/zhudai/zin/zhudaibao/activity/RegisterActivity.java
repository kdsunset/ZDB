...
public class RegisterActivity extends AppCompatActivity {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what){
                case 102: {
                    ...
                    Gson gson = new Gson();
                    LoginResponse registerResultBean;
                    LogUtils.i("执行102");
    ...
    private void initView() {
        ...
        tvTip.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(RegisterActivity.this,ShowAgreementActivity.class));
                finish();
            }
        });
    ...
    private void getInputInfo(){
        ...
        mInvationCode = getEditTextContent(etInvitationCode);
        isAgree=cbAgreeCheck.isChecked()==true?1:0;
        LogUtils.i("输入的手机号："+mUserPhone);
        LogUtils.i("输入的验证码："+mCode);
        LogUtils.i("输入的密码："+mPwd);
        LogUtils.i("输入的确认密码："+mPwdConfirm);
        LogUtils.i("输入的邀请码："+mInvationCode);
        LogUtils.i("是否同意协议："+isAgree);
    ...
    private void attemptLogin() {
       /* if (mAuthTask != null) {
        ...
        etInvitationCode.setError(null);
    ...
    private void postUserInfo() {
        ...
        RequestBody formBody = new FormEncodingBuilder()
                ...
                .add("invitecode", mInvationCode)
                .add("is_gree",isAgree+"")
                .build();
        final Request request = new Request.Builder()
                .url(ZDBURL.DOREGISTER)
                .post(formBody)
                .build();
        client.newCall(request).enqueue(new Callback() {
            @Override
