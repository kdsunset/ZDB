 ...
 */
public class MyPartnerRVAdapter extends RecyclerView.Adapter<MyPartnerRVAdapter.ItemViewHolder> {
    ...
    private List<MyPartnerResponse.Partner> mList;
    private Context mContext;

    private View mHeaderView;

    public MyPartnerRVAdapter(Context context, List<MyPartnerResponse.Partner> list) {
        this.mList = list;
        this.mContext = context;
        int remainder = list.size() % 4;
        MyPartnerResponse.Partner p1=new MyPartnerResponse.Partner(" ");
        MyPartnerResponse.Partner p2=new MyPartnerResponse.Partner(" ");
        MyPartnerResponse.Partner p3=new MyPartnerResponse.Partner(" ");
        if (remainder==1){
            mList.add(p1);
            mList.add(p3);
            mList.add(p2);
    ...
    public ItemViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        final View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_recycleview_mypartner, parent, false);
        ItemViewHolder vh = new ItemViewHolder(view);
        view.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mListener != null) {
                    mListener.OnItemClick(v, (MyPartnerResponse.Partner) view.getTag());//注意这里使用getTag方法获取数据
                }
            }
        });
        return vh;
    ...
    public MyPartnerResponse.Partner getItem(int position) {
    ...
    public void onBindViewHolder(ItemViewHolder holder, final int position) {
        ...
        MyPartnerResponse.Partner dataItem = getItem(position);
        String sex = dataItem.getSex();
        if (!TextUtils.isEmpty(sex)) {
            if (sex.equals("1")) {
                holder.tvPartnerName.setText(dataItem.getRealname() + "先生");
            } else if (sex.equals("2")) {
                holder.tvPartnerName.setText(dataItem.getRealname() + "小姐");
            }
        } else {
            holder.tvPartnerName.setText(dataItem.getRealname() + "");
            LogUtils.i(dataItem.getRealname());
        }
    ...
    public interface OnItemClickListener {
        void OnItemClick(View view, MyPartnerResponse.Partner data);
    }
